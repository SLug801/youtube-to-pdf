...
/**
 * 악보 프레임의 픽셀 연산 모음(모드별). FrameExtractor의 스티칭 루프에서 분리해
 * "특징 추출 / 출력 정리 / 이진화 / 템플릿 매칭 / 노이즈 제거"를 담당한다.
 *
 * 모든 메서드는 입력 Mat을 변형하지 않고 새 Mat(또는 double[])을 돌려주는 순수 연산이라
 * 스티칭 상태(누적 화면·카운터)와 독립적이다. 따라서 단위 테스트/디버그 도구에서
 * 프레임 한 장만 떼어 그대로 호출할 수 있다(OpaqueFrameTest 참고).
 */
final class SheetImageOps {
     ...
     */
    double[] matchOffset(Mat ref, Mat cur, int tw, int inset) {
        int h  = cur.rows();
        int cw = cur.cols();
     ...
     */
    double[] matchOffsetGray(Mat refColor, Mat curColor, int tw, int inset) {

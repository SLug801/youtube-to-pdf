...
final class ScanParams {
    ...
    // CUT 판정은 ROI 전체 폭 상관을 사용한다. 재생 커서로 인한 동일 페이지 오검출은
    // 약 0.87 이상, 반복 리듬이 비슷하지만 실제로
    // 다른 페이지는 약 0.83 이하였다. 둘 사이(0.85)로 갈라 누락과 중복을 함께 막는다.
    static final double CUT_SAME_SCREEN    = 0.85; // 이 미만이면 확정 화면과 다른 화면(전환)

...
기본 변환 엔진은 Python OpenCV Core이며, 기존 Java JAR는 비교·비상 폴백 Worker로 유지합니다.
...
- `YTPDF_JAR_PATH`: Java fat JAR 절대 경로
- `YTPDF_JAVA_COMMAND`: Java 실행 파일, 기본 `java`
- `YTPDF_YTDLP_PATH`: 패키지에 포함된 yt-dlp 경로
- `YTPDF_ENGINE`: `python`(기본), `java`, `auto`
